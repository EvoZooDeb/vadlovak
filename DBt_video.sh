...
if [[ ( "$#" -ne $min_arg ) && ( "$#" -ne $max_arg ) ]]; then
    ...
    echo -e "Necessary arguments:\n   1: <input_model_path>   #Result of 'retinanet-convert-model' command\n   2: <input_video_path>   #Choose video\n   3: <output_file>        #Choose file path & name\nOptional:\n   4: <output_csv_path>    #Create a csv file, which contain the bounding box information: \"x\",\"y\"\n"
    echo "      e.g: ./DBt_video.sh snapshots/model_v1.h5 sample/wildhorse.mp4 wildhorse_with_box.mp4"
    echo "           ./DBt_video.sh snapshots/model_v1.h5 sample/wildhorse.mp4 wildhorse_with_box.mp4 wildhorse_bounding_box.csv"
    exit 1
fi
...
python3 $make_vid $input_model $input_video $draw_boxes $csv_path